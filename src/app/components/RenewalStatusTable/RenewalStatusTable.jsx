...
const RenewalStatusTable = () => {
    const columns = useMemo(() => [
                    ...
                    Cell: ({ renderedCellValue, row }) => (
                        <Box sx={{ display: 'flex', alignItems: 'center', gap: '1rem', paddingLeft: '1rem' }}>
                            <img
                                alt="avatar"
                                height={30}
                                src={row.original.avatar ?? 'https://cloudflare-ipfs.com/ipfs/Qmd3W5DuhgHirLHGVixi6V76LhCkZUz6pnFt5AJBiyvHye/avatar/394.jpg'}
                                loading="lazy"
        ...
        },
        {
            ...
            columns: [
                {
                    accessorFn: (row) => new Date(row.licenseRenewal),
                ...
                },
                {
                    ...
                    accessorFn: (row) => {
                        ...
                        if (row.licenseRenewalDone) return '1Completed';
        ...
        },
        {
            ...
            columns: [
                {
                    accessorFn: (row) => new Date(row.vehicleRenewal),
                ...
                },
                {
                    ...
                    accessorFn: (row) => {
                        ...
                        const currentDate = moment().format('MM-DD-YYYY');
                        const difference = moment(currentDate).diff(date, 'days');
                        if (row.vehicleRenewalDone) return '1Completed';
                        else if (difference > 0) return `2Overdue by ${difference} days`;
                        else if (difference <= 0 && difference >= -30) return `3Due in ${-difference} days`;
                        else if (difference <= -30) return `4Due in ${-difference} days`;
                        else return `5NA`;
